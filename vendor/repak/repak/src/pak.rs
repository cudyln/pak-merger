...
impl PakBuilder {
    ...
    /// Enables or disables concurrent compression and decompression of
    /// independent blocks. Enabled by default.
    pub fn parallel_blocks(mut self, enabled: bool) -> Self {
        self.parallel_blocks = enabled;
        self
    }
    pub fn reader<R: Read + Seek>(self, reader: &mut R) -> Result<PakReader, super::Error> {
        PakReader::new_any_inner(reader, self.key, self.parallel_blocks)
    }
    pub fn reader_with_version<R: Read + Seek>(
        ...
        PakReader::new_inner(reader, version, self.key, self.parallel_blocks)
    }
...
impl PakReader {
    ...
    pub fn read_file<R: Read + Seek, W: Write>(
        ...
        self.read_file_with_parallel_blocks(path, reader, writer, self.parallel_blocks)
    }

    /// Reads one file with an explicit block-parallelism choice, without
    /// rebuilding the Pak reader or reparsing its index.
    pub fn read_file_with_parallel_blocks<R: Read + Seek, W: Write>(
        &self,
        path: &str,
        reader: &mut R,
        writer: &mut W,
...
impl<W: Write + Seek> PakWriter<W> {
    ...
    pub fn write_file(
        ...
        self.write_file_with_progress(path, allow_compress, data, |_, _| true)
    }

    /// Writes one entry while reporting completed logical source bytes.
    /// Returning `false` from `progress` cooperatively cancels the operation.
    pub fn write_file_with_progress<P>(
        &mut self,
        path: &str,
        allow_compress: bool,
        data: impl AsRef<[u8]>,
    ...
    {
        self.write_file_with_progress_and_source_chunks(
            path,
            allow_compress,
            data,
            progress,
    ...
    /// Writes one entry while reporting progress and exposing each logical
    /// source byte exactly once in deterministic source order. The source
    /// callback runs on the calling thread after the corresponding output
    /// bytes have been written, including when compression workers run in
    /// parallel.
    pub fn write_file_with_progress_and_source_chunks<P, S>(
        &mut self,
        path: &str,
        allow_compress: bool,
        data: impl AsRef<[u8]>,
        ...
        S: FnMut(&[u8]),
    {
        self.pak.index.add_entry(
            path.to_string(),
            Entry::write_file_with_source_chunks(
                &mut self.writer,
                self.pak.version,
                ...
                self.parallel_blocks,
                data.as_ref(),
                (&mut progress, &mut source_chunks),
            )?,
        );
...
impl EntryBuilder {
    ...
    pub fn build_entry<D: AsRef<[u8]> + Send + Sync>(
        ...
        build_partial_entry(compression, self.parallel_blocks, data)
    }
}
...
mod test {
    ...
    fn legacy_output(
        ...
        writer
            ...
            .unwrap();
        let output = writer.write_index().unwrap().into_inner();
        (output, progress)
    }
    ...
    fn observed_output(
        ...
        writer
            .write_file_with_progress_and_source_chunks(
                "Data/Observed.uexp",
                allow_compress,
                payload,
                |completed, total| {
                ...
                |chunk| observed.extend_from_slice(chunk),
            )
            .unwrap();
        let output = writer.write_index().unwrap().into_inner();
        (output, progress, observed)
    }
    ...
    #[test]
    fn source_chunks_cover_uncompressed_input_once_without_changing_output() {
        let payload = deterministic_payload(4 * 1024 * 1024 + 257);
        let (legacy, legacy_progress) = legacy_output(PakBuilder::new(), &payload, false);
        let (observed_output, observed_progress, observed) =
            observed_output(PakBuilder::new(), &payload, false);

        assert_eq!(observed_output, legacy);
        assert_eq!(observed_progress, legacy_progress);
        assert_eq!(observed, payload);
        assert_eq!(sha1_bytes(&observed), sha1_bytes(&payload));
    }

    #[cfg(feature = "compression")]
    #[test]
    fn source_chunks_cover_parallel_compressed_input_once_without_changing_output() {
        let payload = deterministic_payload(crate::COMPRESSION_BLOCK_SIZE as usize * 17 + 257);
        let builder = || {
            PakBuilder::new()
                .compression([Compression::Zstd])
                .parallel_blocks(true)
        };
        let (legacy, legacy_progress) = legacy_output(builder(), &payload, true);
        let (observed_output, observed_progress, observed) =
            observed_output(builder(), &payload, true);
