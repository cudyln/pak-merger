...
impl Oodle {
    ...
    /// Returns the maximum output buffer size required by Oodle for `raw_buffer`
    /// bytes. Callers can use this to reserve or preflight storage without
    /// compressing the input twice.
